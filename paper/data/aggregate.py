...
def display_label(paths):
    """Compact paper label with an explicit OS suffix."""
    for kind in ("matrix", "scaling"):
        if kind not in paths:
            continue
        meta = load_json(paths[kind]["json"]).get("metadata", {})
        label = meta.get("label", "?")
        os_name = meta.get("machine", {}).get("os", "")
        if os_name.startswith("macOS"):
            short_os, prefix = "macOS", "Darwin-"
        elif os_name.startswith("Windows"):
            short_os, prefix = "Windows", "Windows-"
        elif os_name.startswith("Linux"):
            short_os, prefix = "Linux", "Linux-"
        else:
            short_os, prefix = os_name.split("-", 1)[0] or "unknown OS", ""
        if prefix and label.startswith(prefix):
            label = label[len(prefix):]
        return f"{label} ({short_os})"
    return "? (unknown OS)"
...
def manifest_table(machines):
    ...
    for mdir in sorted(machines, key=lambda d: machine_label(machines[d])):
        ...
        lines.append(
            f"| {display_label(paths)} | {mc.get('cpu','?')} | {mc.get('os','?')} | "
...
def main():
    ...
    report = ["<!-- GENERATED by paper/data/aggregate.py; do not hand-edit. -->",
              ...
              "**Clock policy:** CPU boost was disabled and clocks were held at "
              "base frequency for every controllable x86 host (all Windows and "
              "Linux runs). Apple Silicon exposes no boost-lock control, so the "
              "Apple M4 Pro (macOS) run used stock clock management; this exception "
              "is reported explicitly rather than being described as boost-disabled.",
              ""]
