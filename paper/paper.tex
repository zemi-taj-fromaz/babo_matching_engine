...
\input{figures/global-pin-architecture}
...
\texttt{matching\_book<SIZE,TRADE\_CAP>} synchronously handles each command.
...
A fixed-capacity inline trade ring records fills without allocating.
...
aggregation, and the trade ring.  The vendored upstream \textit{Liquibook} tests remain a
frozen baseline.
...
workload through the same C ABI, perform the same modify-as-reinsert semantics,
and emit the same report types into an equivalent output path.  The full harness
measures delivery, matching, report production, transport, and the final flush.
Standalone perf binaries also link each order book directly to isolate the core
book from the shared-library boundary.

The final paper will report CPU, topology, memory, operating system, compiler,
build flags, git revision, core affinity, priority status, warm-up policy,
repetitions, aggregation statistic, and run-to-run dispersion.  Correctness and
...
be replaced only by the post-rebuild measurement matrix.

\begin{table}[h]
\centering
\caption{Final full-harness throughput. All rows must pass report hashing; values
await the clean canonical run.}
\label{tab:results}
\scriptsize
\begin{tabular}{@{}lrrrl@{}}
\toprule
